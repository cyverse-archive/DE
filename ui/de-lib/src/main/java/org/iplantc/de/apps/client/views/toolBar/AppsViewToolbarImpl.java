...
public class AppsViewToolbarImpl extends Composite implements AppsToolbarView
{
	...
	@Override
	public HandlerRegistration addAppSearchResultLoadEventHandler(
			AppSearchResultLoadEventHandler handler)
	{
		return addHandler(handler, TYPE);
	}

	@Override
	public HandlerRegistration addBeforeAppSearchEventHandler(
			BeforeAppSearchEvent.BeforeAppSearchEventHandler handler)
	{
	...
	@Override
	public HandlerRegistration addCopyWorkflowSelectedHandler(
			CopyWorkflowSelected.CopyWorkflowSelectedHandler handler)
	{
		return addHandler(handler, CopyWorkflowSelected.TYPE);
	}

	@Override
	public HandlerRegistration addCreateNewAppSelectedHandler(
			CreateNewAppSelected.CreateNewAppSelectedHandler handler)
	{
		return addHandler(handler, CreateNewAppSelected.TYPE);
	}

	@Override
	public HandlerRegistration addCreateNewWorkflowSelectedHandler(
			CreateNewWorkflowSelected.CreateNewWorkflowSelectedHandler handler)
	{
		return addHandler(handler, CreateNewWorkflowSelected.TYPE);
	}

	@Override
	public HandlerRegistration addDeleteAppsSelectedHandler(
			DeleteAppsSelected.DeleteAppsSelectedHandler handler)
	{
	...
	@Override
	public HandlerRegistration addEditWorkflowSelectedHandler(
			EditWorkflowSelected.EditWorkflowSelectedHandler handler)
	{
		return addHandler(handler, EditWorkflowSelected.TYPE);
	}

	@Override
	public HandlerRegistration addRequestToolSelectedHandler(
			RequestToolSelected.RequestToolSelectedHandler handler)
	{
	...
	@Override
	public HandlerRegistration addShareAppSelectedHandler(
			ShareAppsSelected.ShareAppsSelectedHandler handler)
	{
		...
		switch (currentSelection.size())
		{
			...
			case 1:
				...
				shareWithCollaboratorsMiEnabled =
						selectedApp.getPermission() != null && selectedApp.getPermission()
								.equals(PermissionValue.own) && !selectedApp.getAppType()
	...
	boolean containsSharableApps(List<App> apps)
	{
		for(App a : apps)
		{
			if(!(a.getPermission() != null && a.getPermission().equals(PermissionValue.own) && !a
					.getAppType().equals(App.EXTERNAL_APP)))
			{
				return false;
			}
		}
