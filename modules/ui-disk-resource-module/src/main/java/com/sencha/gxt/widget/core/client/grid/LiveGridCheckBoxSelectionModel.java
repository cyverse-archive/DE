...
/**
 * A checkbox selection model which supports selection persistence for LiveGridViews. This class
 * can only be used with {@code Grid}s whose view is a {@code LiveGridView}. The {@link #bindGrid(Grid)}
 * method will throw an error otherwise. Also, the supported number of individual selections is
 * limited to the bound {@code LiveGridView}'s cache size.
 * <p/>
 * This effectively gives this selection model two different states; when the total number of
 * items in the bound collection (determined by
 * {@link PagingLoader#getTotalCount()}) <b>does not</b> exceed the {@code LiveGridView} cache size,
 * and when the total number of items <b>does</b> exceed the cache size.
 * <p/>
 * <h2>Operation when total number of items <u><i>does not exceed</i></u> cache size</h2>
 * When the user selects individual items, they will remain selected regardless of whether they
 * are visible or not.
 * <br/>
 * If the select all checkbox is enabled, all items will be selected,
 * and attempts to retrieve the selection will reflect what is visually selected. Classes which
 * utilize this selection model <b>must not</b> rely on {@link #getSelectedItems()} or
 * {@link #getSelection()} returning all items which are visually selected. See next section for
 * details.
 * <p/>
 * <h2>Operation when total number of items <u><i>exceeds</i></u> cache size</h2>
 * Individually selected items will be persisted as before, unless the number of selected items
 * exceeds the cache size. When this happens, the user will be prevented from making selections
 * which will result in a selection size exceeding the cache size.
 * <br/>
 ...
 * responsibility of classes using this selection model to check whether the select all checkbox
 * is or is not selected, and act accordingly. For all intents and purposes,
 * the select all checkbox should be considered a selection which covers the source of the
 * currently bound selection (i.e. the contents of a selected
 * {@link org.iplantc.de.client.models.diskResources.Folder})
 * <br/>
 * If the user clicks the select all checkbox, and then tries to de-select a single item (which
 * would conceptually result in a selection greater than the cache size) the de-selection will be
 * prevented.
 *
 * @author jstroot
 ...
 */
public class LiveGridCheckBoxSelectionModel extends CheckBoxSelectionModel<DiskResource> implements LiveGridViewUpdateEvent.LiveGridViewUpdateHandler {
  ...
  public void bindGrid(Grid<DiskResource> grid) {
    checkArgument(grid.getView() instanceof LiveGridView,
                  "the grid's view must be a LiveGridView");
    super.bindGrid(grid);
  ...
  /**
   * Return the number of currently selected items. If the select all checkbox is checked and the
   * total number of items exceeds the bound {@link LiveGridView#getCacheSize()},
   * then the total number of items will be returned. Otherwise,
   * the {@link #selected} collection's size will be returned.
   *
  ...
  protected void updateHeaderCheckBox() {
    ...
    if (isSupportedSelectionSize()) {
      ...
      setChecked((sizeMinusFiltered != 0)
                     && (selected.size() == sizeMinusFiltered));
    } else {
              /*
               * Do nothing.
               * If the live view has more items than selections, than we should not change the
